 ...
 * 
 * 1. Add useful parameters on SmartDashboard (i.e velocity and vision control PIDF gains)
 * 2. Initialize power distribution panel.
 * 3. Initialize all CAN Spark Max brushless motor controllers
 *      3a. Set follower modes for 1 motor on the left and right side of the differential drive
 *      3b. Initialize PID controllers (for closed-loop velocity mode)
 *      3c. Initialize encoder controllers
 *      3d. Set position and velocity conversion factors (so that ft or ft/s can be sent directly to motors)
 *      3e. Set output ranges (percent output, max speed, and max acceleration)
 *      3f. Set P and FF (feed forward) gains on the PID controllers
 * 4. Initialize drivetrain peripherals (i.e Spike and gyro)
 */
...
/**
 * @brief Set the default command for the DriveTrain subsystem
 * 
 * The default command is TeleopDrive
 */
...
/**
 * @brief Periodic method for DriveTrain
 * 
 * Constantly update robot velocity on SmartDashboard
 */
...
/**
 * @brief Percent Output (Voltage) Tank Drive
 * 
 * @param left (percent output for left wheels)
 * @param right (percent output for right wheels)
 */
...
/**
 * @brief Percent Output (Voltage) Arcade Drive
 * 
 * @param xSpeed (the percent output to send to both left/right wheels)
 * @param zRotation (the percent output to rotate the robot)
 */
...
/**
 * @brief Velocity Tank Drive using closed-loop PIDF velocity control with CAN Spark Maxes
 * 
 * Velocity is in ft/s
 * Note: utilizes smart accel variable to prevent jerky driving
 * 
 * @param left (the velocity to send to the left wheels)
 * @param right (the velocity to send to the right wheels)
 * @param squaredInputs (gives extra motor control at full power)
 */
...
/**
 * @brief Tank Drive velocity with vision error integration
 * 
 * Add/subtract velocity depending on kP_Vision and error from 
 * VisionTable NetworkTable
 * 
 * @param velocity [ft/s]
 * @param error [pixels]
 */
